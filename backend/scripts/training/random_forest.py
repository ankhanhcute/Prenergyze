...
Extracted from random_forest_regression.ipynb and standardized.
"""
import os
import pickle
import time
...
def train_random_forest(evaluate_cv: bool = True, save_model: bool = True, use_grid_search: bool = True):
    """
    ...
    if use_grid_search and 'best_params' in results:
        # Use most common best params
        from collections import Counter
        param_counts = Counter(str(p) for p in results['best_params'])
        most_common = eval(param_counts.most_common(1)[0][0])
        print(f"Using best parameters: {most_common}")
        model = RandomForestRegressor(**most_common, random_state=42, n_jobs=-1)
    else:
        model = RandomForestRegressor(n_estimators=200, max_features='sqrt', random_state=42, n_jobs=-1)
...
if __name__ == '__main__':
    train_random_forest(evaluate_cv=True, save_model=True, use_grid_search=True)
